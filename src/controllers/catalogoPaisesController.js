...
async function listar(req, res) {
  try {
    ...
    const resultado = await query(
      `SELECT id, clave, nombre, bandera_emoji, estado, normativa_principal, descripcion_estado, activo, orden
...
// ------------------------------------------------------------
const CAMPOS_EDITABLES = ['nombre', 'bandera_emoji', 'estado', 'normativa_principal', 'descripcion_estado', 'orden', 'activo'];
...
async function actualizar(req, res) {
  ...
  const asignaciones = columnas.map((c, i) => `${c} = $${i + 2}`);
