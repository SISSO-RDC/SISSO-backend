...
// NOTA (Auditoria N.15): las variantes "_VACIA"/"_VACIOS" de
// matrizRiesgos, ergonomia, aptitudMedica, coberturaExamenes,
// hallazgosAnormales y consentimientos se eliminaron de aqui porque
// ya no se usan -- proyectarIndicadoresSegunRol() ahora OMITE la
// clave en vez de enviar un placeholder vacio (ver su comentario).
// EPIDEMIOLOGIA_VACIA se conserva: sigue en uso para la redaccion
// por k-anonimato de grupos pequeños (linea ~400), que es un caso
// distinto (el rol SI esta autorizado a ver la seccion; se redacta
// el contenido, no se retira la clave).
 ...
 * matriz completa.
 *
 * CORREGIDO en Auditoria N.15 (regresion detectada al ejecutar la
 * suite completa de pruebas: 6 pruebas de contrato RBAC de
 * G-N08-01 fallaban). El 26/08/2026 se habia cambiado esta funcion
 * para no omitir la clave de cada seccion restringida -- en su
 * lugar se devolvia un placeholder con la MISMA forma que la
 * seccion real pero en cero, marcado con `_restringido: true`. La
 * intencion (evitar que indicadores.js truene al leer
 * `datos.matrizRiesgos.total` quando la clave no existe) era
 * legitima, pero el resultado reabria en la practica el hallazgo
 * G-N08-01: un rol no autorizado volvia a recibir la clave
 * `aptitudMedica` (aunque vacia), revelando su forma exacta y
 * rompiendo el contrato de minimizacion de datos que la auditoria
 * exige ("el dato no debe viajar", no "el dato debe viajar vacio").
 *
 * La correccion ahora es la correcta en ambos sentidos: la clave se
 * OMITE por completo para quien no la debe ver (como antes del
 * 26/08), y se agrega `_seccionesNoDisponibles` (array de NOMBRES de
 * seccion, sin su forma ni su contenido) para que el frontend pueda
 * distinguir "no autorizado para tu rol" de "sin datos todavia" sin
 * que el backend tenga que enviar la seccion misma. indicadores.js
 * fue actualizado en esta misma entrega para renderizar cada seccion
 * solo si la clave esta presente.
 */
function proyectarIndicadoresSegunRol(indicadores, rol) {
  ...
  if (rol === 'medico') {
    return {
      ...
      _seccionesNoDisponibles: ['matrizRiesgos', 'ergonomia'],
    };
  }
  if (rol === 'sso') {
    return {
      totalTrabajadores, coberturaEmo, coberturaExamenes, matrizRiesgos, ergonomia, consentimientos,
      _seccionesNoDisponibles: ['aptitudMedica', 'hallazgosAnormales', 'epidemiologia'],
    };
  }
  if (rol === 'th') {
    return {
      totalTrabajadores, coberturaEmo,
      _seccionesNoDisponibles: ['aptitudMedica', 'coberturaExamenes', 'hallazgosAnormales', 'matrizRiesgos', 'ergonomia', 'consentimientos', 'epidemiologia'],
    };
  }
  ...
  return {
    ...
    _seccionesNoDisponibles: ['aptitudMedica', 'hallazgosAnormales', 'ergonomia', 'epidemiologia'],
  };
}
